...
def view_single_dataset(dataset_name, model_name_pattern=None, detailed=False, mode="pairwise"):
    ...
    file_path = os.path.join(model_name_pattern, dataset_name, f"{mode}.jsonl")
...
def view_all_datasets(model_name_pattern=None, detailed=False, mode="pairwise"):
    """查看所有五个数据集的结果"""
    print("="*80)
    print("Correctness数据集评测结果汇总")
    print("="*80)
    
    results = []
    
    for dataset in CORRECTNESS_DATASETS:
        result = view_single_dataset(dataset, model_name_pattern, detailed, mode)
        if result:
    ...
    if results:
        ...
        for r in results:
            ...
            if accuracy is not None:
                print(f"{dataset:<25} {accuracy:.4f} ({accuracy*100:.2f}%)  {file_path}")
                total_acc += accuracy
                valid_count += 1
            else:
                print(f"{dataset:<25} N/A              {file_path}")
        
        if valid_count > 0:
            avg_accuracy = total_acc / valid_count
            print("-" * 80)
            print(f"{'平均准确率':<25} {avg_accuracy:.4f} ({avg_accuracy*100:.2f}%)")
            print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description='查看和汇总correctness数据集（五个best_of_k数据集）的评测结果',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 查看所有数据集的结果（默认行为）
  python view_correctness_results.py
  python view_correctness_results.py --summary
  
  # 通过模型路径查找结果
  python view_correctness_results.py grm-pub-1109-rel-2200word-bf-global_step_400
  
  # 通过模型名称模式查找结果
  python view_correctness_results.py --model-name dx-32b
  
  # 只查看指定数据集的结果
  python view_correctness_results.py --dataset mmlu_pro_best_of_k
  
  # 显示详细信息
  python view_correctness_results.py --detailed
  
  # JSON格式输出
  python view_correctness_results.py --json
        """
    )
    parser.add_argument('result_path', nargs='?', type=str,
                       help='模型路径（可选，用于自动匹配结果文件名）')
    # parser.add_argument('--model-name', type=str,
    #                    help='模型名称模式（用于匹配结果文件名，优先级高于model_path）')
    parser.add_argument('--output-dir', type=str, default='data', 
                       help='结果文件目录（默认: data）')
    parser.add_argument('--summary', action='store_true', 
                       help='汇总所有数据集的结果（默认行为）')
    parser.add_argument('--detailed', action='store_true', help='显示详细信息')
    parser.add_argument('--json', action='store_true', help='以JSON格式输出')
    parser.add_argument('--dataset', type=str, choices=CORRECTNESS_DATASETS, 
                       help='只查看指定数据集的结果')
    parser.add_argument("--mode", default="pairwise", type=str)
    
    args = parser.parse_args()
    model_name_pattern = args.result_path
    # # 确定使用的模型名称模式
    # model_name_pattern = args.model_name
    # if not model_name_pattern and args.model_path:
    #     # 如果没有指定 --model-name，但从位置参数提供了模型路径，则从路径中提取
    #     model_name_pattern = args.model_path
    ...
    if args.dataset:
        # 只查看单个数据集
        result = view_single_dataset(args.dataset, model_name_pattern, args.detailed)
        if args.json:
            print(json.dumps(result, indent=2, ensure_ascii=False))
    else:
        # 查看所有数据集（默认行为，--summary 只是明确指定）
        results = view_all_datasets(model_name_pattern, args.detailed, args.mode)
        if args.json:
