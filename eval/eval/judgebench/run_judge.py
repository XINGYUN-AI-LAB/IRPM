...
async def judge_pairs(pairs: List[Dict[str, Any]], judge_name: str, judge_model: str, concurrency_limit: int = 1, reverse_order: int = False, output_file: str = None, device_map=None):
    semaphore = asyncio.Semaphore(concurrency_limit)
    judge = judges.get_judge_from_judge_name_and_model(judge_name, judge_model, device_map=device_map)
...
def main(args: argparse.Namespace) -> None:
    ...
    if pairs: 
        ...
        if hasattr(args, 'device_map') and args.device_map:
            if args.device_map.lower() == "auto":
                device_map = "auto"
            elif args.device_map.isdigit():
                # 如果是数字，使用 "auto" 模式（transformers 会自动分配到所有可用 GPU）
                # 或者可以设置环境变量 CUDA_VISIBLE_DEVICES 来限制使用的 GPU
                device_map = "auto"
                num_gpus = int(args.device_map)
                # 设置可见的 GPU
    ...
    if failed_pairs:
        print(f"\n⚠️  Warning: {len(failed_pairs)} pairs failed to get judgments (out of {len(pairs)} total)")
        print(f"   Failed pair IDs: {failed_pairs[:10]}{'...' if len(failed_pairs) > 10 else ''}")
