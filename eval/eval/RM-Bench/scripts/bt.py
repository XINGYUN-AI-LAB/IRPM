...
class EvaluateProject():
    ...
    def load_eval_data(self, filename):
        data = [json.loads(line) for line in open(filename, 'r') if line.strip()]
        print(f"eval data len is {len(data)} ")
        return data
    ...
    def pipline2(self, eval_data=None):
        if eval_data is None:
            eval_data = self.load_eval_data(args.eval_file)

        outputs = []
        for i, d in enumerate(eval_data[:5]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="", help="oss path")
